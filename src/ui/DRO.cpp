...
constexpr uint32_t DRO::JOG_DISTS[];
...
void DRO::drawContents() {
    ...
    auto jd = (int32_t) JOG_DISTS[cDist];
    snprintfloat(str, LEN, jd, jd > 1000 ? 2 : 1,4 );
...
void DRO::onButtonAxes(int bt, Evt evt) {
    if (evt == Evt::DOWN || evt == Evt::HOLD) {
        int axis = -1;
        auto d = (int32_t) JOG_DISTS[cDist];
        uint16_t f = JOG_FEEDS[cFeed];
        switch (bt) {
            case Display::BT_L:
                axis = 0;
                d = -d;
...
void DRO::onButtonShift(int bt, Evt evt) {
    ...
    switch (bt) {
        ...
        case Display::BT_ZUP: {
            ...
            if (speed != 0) {
                char t[15];
                size_t l = snprintf(t, 15, "M3 S%d", speed);
...
void DRO::drawAxis(char axis, int32_t value, int x, int y) {
    static const int LEN = 13;
    static char buffer[LEN];
    buffer[0] = axis;
    snprintfloat(buffer + 1, LEN - 1, value, defaultAxisPrecision, 5 + defaultAxisPrecision);
