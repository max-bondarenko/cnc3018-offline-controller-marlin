...
bool MarlinDevice::jog(uint8_t axis, int32_t dist, uint16_t feed) {
...
void MarlinDevice::parseOk(const char* input, size_t len) {
    ...
    while (fromMachine != nullptr) {
        switch (fromMachine[0]) {
            case 'T':
                hotendTemp = parseFloatInMils((fromMachine + 2));
                nextTemp = true;
                break;
            case 'B':
                if (fromMachine[1] == '@') {
                    bedPower = parseFloatInMils((fromMachine + 3));
                } else {
                    bedTemp = parseFloatInMils((fromMachine + 2));
                    nextBedTemp = true;
                }
                break;
            case 'X':
                x_mil = parseFloatInMils((fromMachine + 2));
                break;
            case 'Y':
                y_mil = parseFloatInMils((fromMachine + 2));
                break;
            case 'Z':
                z_mil = parseFloatInMils((fromMachine + 2));
                break;
            case 'E':
                e = parseFloatInMils((fromMachine + 2));
                break;
            case '@':
            ...
            case '/':
                if (nextTemp) {
                    hotendRequestedTemp = parseFloatInMils((fromMachine + 1));
                } else if (nextBedTemp) {
                    bedRequestedTemp = parseFloatInMils((fromMachine + 1));
                }
                nextTemp = false;
                nextBedTemp = false;
            default:
