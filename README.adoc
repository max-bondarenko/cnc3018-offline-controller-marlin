...
N.B. CMake 3.31 added --dependency-file flag to link options which is not supported by arm gcc-ld.
Do not update arm-gcc to 10.x, it has major changes in link optimization. It significantly increases elf size by
adding cp-demangle & unwind despite *-fno-except* flag. *I'm using old cmake as workaround.*
